  ...
  <div class="searchproduct">
    <div class="breadbox">
      <Breadcrumb separator=">">
        <BreadcrumbItem :to="getLOCATIONORIGIN">Starday</BreadcrumbItem>
        <BreadcrumbItem :to="getLOCATIONORIGIN + 'sitemap/searches/1'">Searches</BreadcrumbItem>
    ...
    <div class="searchbox">
      ...
      <section :class="sorted*1===0?'activesort':''" @click="changeSortHandle(0,'desc')">
        {{ getLanguageData.searchPage.sale }}
      </section>
      <section :class="sorted*1===5?'activesort':''" @click="changeSortHandle(5,'desc')">
        {{ getLanguageData.searchPage.credit }}
      </section>
      <section :class="sorted*1===1?'activesort':''" @click="changeSortHandle(1,'asc')">
        <span>{{ getLanguageData.searchPage.price }}</span>
        <div class="imgupdown">
          <div class="imgupdownbox">
            <div class="uping">
              <img v-if="sorted*1===1 && sortOrder==='asc'" src="@/assets/images/store/upac.png" alt="">
              <img v-else src="@/assets/images/store/up.png" alt="">
            </div>
            <div class="downimg" @click="changeSortHandle(1,'desc')">
              <img v-if="sorted*1===1 && sortOrder==='desc'" src="@/assets/images/store/downac.png" alt="">
    ...
    <div class="searchcontent">
      <likegood v-for="(item,index) in listData" :key="index" :goods-item="item" />
    </div>
    ...
    <div v-if="tagData && tagData.tags && tagData.tags.length" class="releasesearchbox">
      ...
      <a v-for="(item,index) in tagData.tags" :key="index" :href="getLOCATIONORIGIN + 'search/product/'+ item + '/1'">{{ item }}</a>
    </div>
  </div>
</template>
...
export default {
  ...
  async asyncData ({ $axios, params, query, error }) {
    try {
      ...
      const { data } = await $axios.post('starday-elasticsearch/search/goods', {
        keyword: params.productwd || '',
        pageNo: params.productpageno || 1,
      ...
      // 是否存在关联词tag
      const tagData = await $axios.post('/starday-elasticsearch/search/tags', { keywords: params.productwd, sortOrder: (new Date().getTime() / 1000) - [1, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000, 10000000000][Math.floor(Math.random() * 10)] })
      ...
      const seoData = {
        title: (params.productwd || '') + 'の通販「人気' + (params.productwd || '') + '」商品おすすめ｜Starday',
        keyWords: (params.productwd || '') + ',' + (params.productwd || '') + '通販,starday,starday市場,starday mart,通販,インターネットショッピング,オンラインショッピング"',
        des: (params.productwd || '') + '通販のページ、人気の' + (params.productwd || '') + 'がお得な価格で登場！全品最安値99円から！セール商品・送料無料商品も多数！stardayは総合オンラインショッピングサイトです。'
      }
      return {
      ...
      link: [
        {
          hid: 'canonical',
          ref: 'canonical',
          href: 'https://www.starday.jp/search/product/' + vm.keyword + '/1'
        },
        {
          hid: 'alternate',
          ref: 'alternate',
          media: 'only screen and (max-width: 640px)',
          href: 'https://m.starday.jp/search/product/' + vm.keyword + '/1'
        }
      ]
    }
  },
  ...
  methods: {
    ...
    changeSortHandle (num, str) {
      this.$router.push('/search/product/' + this.keyword + '/1' + '?sorted=' + num + '&sortOrder=' + str)
    },
    ...
    // },
    absolutepathFilter () {
      const _path = 'search/product/' + this.keyword + '/pageNo'
      const _hasharr = []
